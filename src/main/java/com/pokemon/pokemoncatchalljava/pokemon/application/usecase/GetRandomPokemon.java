...
@Slf4j
@AllArgsConstructor
public class GetRandomPokemon implements GetRandomPokemonInputPort {

    private final RandomComponent random;
    private final PokemonOutputPort pokemonOutputPort;

    @Override
    public Either<Exception, PokemonDomain> execute() {
        ...
        return Try.of(random::randomizeInt)
                ...
                .get()
                .peek(
                        (pokemonDomain) -> log.info("Pokemon Got {}", pokemonDomain)
                ).peekLeft(
                        (e) -> log.error("error getting pokemon {} ", e.getMessage())
                );
